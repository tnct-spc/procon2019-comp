...
Field Field::generateRandomField(Point size, size_t agent_count, int min_value, int max_value, double minus_per){
    ...
    if(is_x_symmetry == 0 ,1){
     random_x_size = is_x_symmetry ? (size.x + 1) / 2 : size.x;
     random_y_size = is_x_symmetry ? size.y : (size.y + 1) / 2;
     }else {
        random_x_size =  (size.x + 1) / 2;
        random_y_size =  size.y ;
     }
